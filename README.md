...
#### Este proyecto utiliza datos públicos de incidencia delictiva de México, la fuente de datos crudos son del SESNSP: https://www.gob.mx/sesnsp/acciones-y-programas/datos-abiertos-de-incidencia-delictiva
...
Este proyecto demuestra un flujo de datos de **extremo a extremo (end-to-end)** para transformar datos públicos de incidencia delictiva en un dashboard interactivo y confiable. El objetivo fue convertir datos crudos y dispersos en una **fuente de verdad** para el análisis estratégico, aplicando las mejores prácticas de la ingeniería de datos y la inteligencia de negocios.

---
### El Pipeline

El corazón del proyecto es una arquitectura **Medallion** implementada en **Microsoft Fabric**. Este enfoque nos permitió procesar los datos de manera estructurada y escalable, siguiendo un patrón de **ELT** (Extract, Load, Transform).

* **Bronze 🥉 (Ingesta):** Los datos crudos de incidencia delictiva, obtenidos directamente de una fuente pública oficial de **SharePoint**, se cargaron en esta capa para mantener una copia fiel del origen.
...
* **Gold 🥇 (Análisis):** En esta etapa final, los datos limpios se agregaron y modelaron en un **esquema de estrella (star schema)**, optimizado para el consumo en el reporte de Power BI. Este modelo se compone de una **tabla de hechos** y **tablas de dimensiones** que garantizan un rendimiento superior para el análisis y la exploración de datos.

---
### Resultados y Herramientas

El resultado final es un dashboard en **Power BI** que ofrece una visión clara y confiable de las tendencias de incidencia delictiva. Para la capa de visualización, se aplicaron principios de **diseño UX (experiencia de usuario)** para asegurar que el reporte sea intuitivo, fácil de navegar y que comunique los insights de manera efectiva a los usuarios finales.
...
* **Componentes de Fabric:** Lakehouse, Notebooks de PySpark.
* **Lenguajes:** PySpark, SQL.
